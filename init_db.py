...
# Check if card_discount column exists, add it if missing
def add_card_discount_column_if_missing(conn):
    cursor = conn.cursor()
    # Check if card_discount column exists
    cursor.execute("PRAGMA table_info(customers)")
    columns = [column[1] for column in cursor.fetchall()]
    
    if 'card_discount' not in columns:
        print("Adding missing card_discount column to customers table...")
        cursor.execute("ALTER TABLE customers ADD COLUMN card_discount FLOAT DEFAULT 0.0")
        conn.commit()
        print("✅ card_discount column added successfully!")
    else:
        print("✅ card_discount column already exists!")

# Check if address column exists, add it if missing
def add_address_column_if_missing(conn):
    cursor = conn.cursor()
    ...
    if 'address' not in columns:
        print("Adding missing address column to customers table...")
...
try:
    ...
    add_card_discount_column_if_missing(conn)
    add_address_column_if_missing(conn)
    ...
    for customer in customers:
        try:
            ...
            print(f"Skipping duplicate customer: {customer[0]} - {e}")
